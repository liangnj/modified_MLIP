...
bool Commands(const string& command, vector<string>& args, map<string, string>& opts)
{
	...
	BEGIN_UNDOCUMENTED_COMMAND("run",
		...
		MLIP_Wrapper mlip_wrapper(LoadSettings(args[1]));
		Settings driver_settings(opts);
		CfgReader cfg_reader(&mlip_wrapper, driver_settings);
		cfg_reader.Run();
	} END_COMMAND;

	BEGIN_UNDOCUMENTED_COMMAND("merge-cfg",
		"merge files from mpi relaxation etc",
		"merge configuration files filename_0, filename_1, ..., filename_{n_cores-1}\n"
		"mlp merge-cfg filename [options]:\n"
		"  Options can be given in any order. Options include:\n"
		"  --n_cores=<num>: number of cores (if different from the current in mpirun)\n"
		"Usage:\n"
		"mlp merge-cfg filename --n_cores=16\n"
		"or\n"
		"mpirun -n 16 mlp merge-cfg filename\n"
		) {

		if (args.size() != 1) {
			cout << "mlp run: 1 arguments required\n";
			return 1;
		}

		if(mpi_rank == 0) {
			int n_cores = mpi_size;
			if(opts["n_cores"]!= "") {
				try { n_cores = stoi(opts["n_cores"]); }
				catch (invalid_argument) {
					cerr << (string)"mlp error: " + opts["n_cores"] + " is not an integer\n";
				}
			}

			vector<vector<Configuration>> cfgs;
			cfgs.resize(n_cores);
			for(int i=0; i<n_cores; ++i) {
				ifstream ifs(string(args[0])+"_"+to_string(i), ios::binary);
				if(ifs.good())
					cfgs[i] = LoadCfgs(string(args[0])+"_"+to_string(i));
			}

			ios::sync_with_stdio(false); //qwe

			ofstream ofs;
			ofs.open(args[0], ios::binary);
			int max_size = cfgs[0].size();
			int total_size = cfgs[0].size();
			if (total_size != 0) {
				for(int i=1; i<n_cores; ++i) {
					total_size += cfgs[i].size();
					if(max_size < cfgs[i].size())
						max_size = cfgs[i].size();
				}
				for(int j=0; j<cfgs[0].size(); ++j)
					for(int i=0; i<n_cores; ++i)
						if(j < cfgs[i].size()) {
							cfgs[i][j].Save(ofs);
						}
