	...
	for (int ii = 0; ii < inum; ii++) 
	{
		...
		//	Edit begin
			for (int jj = 0; jj < nbh.count; jj++)
			{
				...
				site_virial[i][3] -= p_site_energy_ders[3*jj+1] * nbh.vecs[jj][0];
				site_virial[i][4] -= p_site_energy_ders[3*jj+2] * nbh.vecs[jj][0];
				site_virial[i][5] -= p_site_energy_ders[3*jj+2] * nbh.vecs[jj][1];
				site_virial[i][6] -= p_site_energy_ders[3*jj+0] * nbh.vecs[jj][1];
				site_virial[i][7] -= p_site_energy_ders[3*jj+0] * nbh.vecs[jj][2];
				site_virial[i][8] -= p_site_energy_ders[3*jj+1] * nbh.vecs[jj][2];
		//      0 3 4
		// 	    6 1 5
		// 		7 8 2 rij*Fji
        //      Edit end
